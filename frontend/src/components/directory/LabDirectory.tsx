...
interface LabDirectoryProps {
  onAskAIAboutLab?: (prompt: string) => void;
}
...
export const LabDirectory: React.FC<LabDirectoryProps> = ({ onAskAIAboutLab }) => {
      ...
      <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs sm:text-sm text-slate-300 max-w-2xl mt-0.5">
                Locate BIS Apex Central Laboratories, Regional Test Centers, and recognized commercial labs for sample conformity assessment.
              </p>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-card border border-slate-200 p-4 space-y-3">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search by lab name, city, state, or discipline..."
              value={searchQuery}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredLabs.map((lab) => (
            <div
              key={lab.id}
              className="bg-white rounded-2xl p-5 border border-slate-200 shadow-card hover:border-bis-300 transition-all flex flex-col justify-between"
            >
              <div className="space-y-3">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-[11px] font-medium text-slate-500 flex items-center gap-1">
                    <MapPin className="w-3.5 h-3.5 text-bis-600" />
                    {lab.city}, {lab.state}
                  </span>
                </div>
                ...
                <div className="space-y-1.5 pt-1 text-xs text-slate-600">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-[11px]">{lab.phone}</span>
                  </div>
                  <div className="flex items-center gap-2">
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
                ...
                {onAskAIAboutLab && (
                  <button
                    onClick={() =>
                      onAskAIAboutLab(
                        `What products and standard tests are handled by ${lab.name} in ${lab.city}? What is the sample submission protocol?`
                      )
                    }
                    className="px-3 py-1 rounded-lg bg-bis-50 hover:bg-bis-100 text-bis-800 text-xs font-semibold flex items-center gap-1 transition-colors"
                  >
