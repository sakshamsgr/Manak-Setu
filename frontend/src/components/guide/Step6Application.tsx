...
export const Step6Application: React.FC<Step6ApplicationProps> = ({
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-1">
          <span className="px-2.5 py-0.5 text-[10px] font-extrabold uppercase bg-emerald-100 text-emerald-900 rounded">
            Stage 5 of 5 • Final Step
          </span>
          <h2 className="text-lg sm:text-xl font-extrabold text-slate-900 tracking-tight">
            {t('s6Title') || 'Apply for BIS License'}
          </h2>
          <p className="text-xs sm:text-sm text-slate-500">
            {t('s6Subtitle') || 'Submit your documents, pay the fees, and track your application online.'}
          </p>
        </div>
        <PageInfoButton
      ...
      {/* 5-Stage Completed Certification Journey Summary Card */}
      <div className="p-5 sm:p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-4">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Here is a quick look at everything you need to do
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          {/* Stage 2: Standards & QCO (Merged) */}
          <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1">
            <div className="flex items-center justify-between text-[11px] font-extrabold uppercase text-slate-500">
              <span>Stage 2 • Standard & QCO</span>
            ...
            <p className="text-[11px] text-slate-500 leading-snug line-clamp-1">
              {guideData?.certificationDetails?.isMandatory ? 'Mandatory (QCO)' : 'Voluntary'} • {guideData?.certificationDetails?.scheme || 'Scheme-I'}
            </p>
          </div>

          {/* Stage 3: Testing & Labs */}
          <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1">
            <div className="flex items-center justify-between text-[11px] font-extrabold uppercase text-slate-500">
              <span>Stage 3 • Testing & Labs</span>
              <FlaskConical className="w-3.5 h-3.5 text-bis-700" />
            </div>
            <div className="font-bold text-xs sm:text-sm text-slate-900 truncate">
              {(guideData?.testingDetails?.routineTests?.length || 0) + (guideData?.testingDetails?.requiredTests?.length || 0)} Safety Tests
            </div>
            <p className="text-[11px] text-slate-500 leading-snug">
              {guideData?.testingDetails?.laboratories?.length ? `${guideData.testingDetails.laboratories.length} Approved Labs` : 'In-House & BIS Labs'}
            </p>
          </div>
          ...
          <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1">
            <div className="flex items-center justify-between text-[11px] font-extrabold uppercase text-slate-500">
              <span>Stage 4 • Documents</span>
              <FolderCheck className="w-3.5 h-3.5 text-bis-700" />
            </div>
            <div className="font-bold text-xs sm:text-sm text-slate-900 truncate">
              {guideData?.documentChecklist?.length || 6} Required Files
            </div>
            <p className="text-[11px] text-slate-500 leading-snug">
              Form-V, Factory Maps, & Machine Lists
            </p>
          </div>
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-gradient-to-r from-bis-950 via-bis-900 to-bis-800 text-white shadow-md flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="space-y-2 max-w-2xl">
          ...
          <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
            You will use the official e-BIS website to submit your forms, pay government fees, and track your application progress.
          </p>
        </div>
        ...
        >
          <span>{t('s6ManakonlineCTA') || 'Open e-BIS Website'}</span>
      ...
      <div className="space-y-3">
        <div className="text-xs font-extrabold uppercase tracking-wider text-slate-800">
          {t('s6TimelineTitle') || 'What Happens After You Apply?'}
        </div>
              ...
              <div className="p-5 rounded-2xl bg-white border border-slate-200 space-y-2 group-hover:border-bis-300 transition-colors shadow-xs">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <span className="px-2.5 py-1 text-xs font-semibold bg-slate-50 border border-slate-200 rounded-md text-slate-600 font-mono flex items-center gap-1.5">
                    ...
                    <span>{step.timeline || 'Estimated Time'}</span>
                  </span>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-100 text-xs">
                  <div className="flex items-center gap-1.5 text-bis-900 font-semibold">
                    ...
                    <span>{step.action || 'Wait for updates on portal'}</span>
                  </div>

                  <div className="flex items-center gap-3 text-[11px] text-slate-500">
                    {step.responsibleParty && (
                      <span><strong>Who does this: </strong>{step.responsibleParty}</span>
                    )}
                    {step.feeAmount && (
      ...
      {onAskAI && (
        <div className="px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-between gap-3 flex-wrap">
          <p className="text-xs text-slate-600">
            Have questions about how to use the portal or how long the license takes?
          </p>
          <button
            type="button"
            onClick={() =>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-slate-200">
        ...
        >
          ...
          <span>{t('prevStepBtn') || 'Previous: Documents'}</span>
        </button>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span>{t('printGuideBtn') || 'Print Guide'}</span>
          </button>
          ...
          >
            ...
            <span>{t('s6RestartBtn') || 'Start New Search'}</span>
          </button>
        </div>
      </div>
    </div>
